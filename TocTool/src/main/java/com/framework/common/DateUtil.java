...
public class DateUtil {
	 ...
	 * @return
	 */
	public static Date valueOfStandards(String s) {
	 ...
	 * @return
	 */
	public static Date frontMonth(Date date) {
		GregorianCalendar gc = new GregorianCalendar();
		gc.setTime(date);
		gc.add(Calendar.MONTH, -1);
		return roundToDay(gc.getTime());
	...
	public static Date afterNumDate(Date date, int num) {
		GregorianCalendar gc = new GregorianCalendar();
		gc.setTime(date);
	...
	public static int getLeaoYear(int year, int month) {
		/**
		 * * 控制塔输入一个年约份，判断对应的这个月有多少天 分析：1.要判断这一年是不 是闰年 2.要判断这个月份 *
		 * 判断闰年的规则是：四年一闰，百年 不闰，四百年再闰 * 闰年的2月是29天 非闰年的2月是28天 *
		 */
		int days = 0;
		switch (month) {
		case 1:
	...
	public static void main(String[] args) throws ParseException {
		Date date=new Date();
		String begintime=DateUtil.format(date, "yyyy-MM-dd");
		System.out.println(begintime);
		//获取前一个月
		Date lastDate=DateUtil.frontMonth(date);
		String endtime=DateUtil.format(lastDate, "yyyy-MM-dd");
		String [] str=endtime.split("-");
		String beginyear=str[0];
		String endmonth=str[1];
		System.out.println(beginyear+"--"+endmonth);
		int year=Integer.parseInt(beginyear);
		int month=Integer.parseInt(endmonth);
		int days=DateUtil.getLeaoYear(year, month);
